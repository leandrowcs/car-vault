...
const distanceLabel = (km: number, unit: 'km' | 'mi') => `${Math.round(unit === 'mi' ? km / 1.609344 : km).toLocaleString('en-CA')} ${unit}`
...
export function ActivityTimeline() {
  ...
  if (!months.length) return <p className="timeline-empty">No transactions yet. Add your first fill-up, expense or service.</p>
  return <>
    <div className="activity-timeline">{months.slice(0, monthLimit).map(month => <section key={month.month} aria-label={month.month}>
      <div className="timeline-month">
        <h4>{new Date(`${month.month}-01T12:00:00`).toLocaleDateString('en-CA', { month: 'long', year: 'numeric' })}</h4>
        <div><strong>{formatCurrency(month.total, settings.currency)}</strong><span> · {month.entries.length} transactions</span></div>
        {month.consumption !== null && <p>{distanceLabel(month.distance, settings.distanceUnit)} · {economyLabel(month.consumption, settings.fuelEconomyUnit)} <small>(full-tank intervals)</small></p>}
        {month.refillInterval && <p>Fill-ups every {month.refillInterval.days} days · about {distanceLabel(month.refillInterval.km, settings.distanceUnit)}</p>}
      </div>
      <ol className="timeline-list">{month.entries.map(entry => {
        ...
        return <li key={entry.id} className="timeline-entry">
          ...
          <div className="timeline-description"><h4>{entry.title}</h4><p>{entry.detail}</p><div className="timeline-metrics">
    ...
    {months.length > monthLimit && <button className="btn btn-secondary timeline-more" type="button" onClick={() => setMonthLimit(n => n + 3)}>Show earlier months</button>}
  </>
}
